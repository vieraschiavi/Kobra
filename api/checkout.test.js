...
test("aviso: incluye país e IP cuando el proxy los manda", async () => {
  process.env.MP_ACCESS_TOKEN = "TEST-token";
  process.env.RESEND_API_KEY = "re_test";
  const mails = mockConResend(mpOk);
  const checkout = require("./checkout");
  const conGeo = reqIP("200.40.9.1", { plan: "pro" });
  conGeo.headers["x-vercel-ip-country"] = "UY";
  await checkout(conGeo, res());
  assert.equal(mails.length, 1);
  assert.match(mails[0].text, /Pa[íi]s:\s+UY/i);
  assert.match(mails[0].text, /IP:\s+200\.40\.9\.1/);
});

test("aviso: sin país/IP en las cabeceras, el mail sale igual", async () => {
  process.env.MP_ACCESS_TOKEN = "TEST-token";
  process.env.RESEND_API_KEY = "re_test";
  const mails = mockConResend(mpOk);
  const checkout = require("./checkout");
  const r = res();
  await checkout(reqIP("", { plan: "pro" }), r);
  assert.equal(r.statusCode, 200);
  assert.equal(mails.length, 1, "la falta de geo no puede tumbar el aviso");
});

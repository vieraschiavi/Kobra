...
if not "!PYEXE!"=="" (
  ...
  rem C: - sin importar que disco hayas elegido para todo lo demas. Con C:
  rem justo de espacio (el motivo original de este .bat), instalar ahi podia
  rem volver a fallar por lo mismo que se vino a arreglar. TargetDir es una
  rem propiedad documentada del instalador y funciona con o sin InstallAllUsers.
  ...
  rem falta: este mismo .bat ya guarda la ruta exacta a PYEXE.
  ...
  rem si no aparecia ahi, PYEXE quedaba vacio y el .bat salia por el camino de
  rem abajo.
  for %%P in (
    "!PYDIR!\python.exe"
  ...
  if not defined PYEXE (
    ...
    echo       https://www.python.org/downloads/ marcando "Add Python to PATH",
    echo       y volve a ejecutar este .bat: lo detecta solo.
    echo.
    rem exit /b 1 y NO /b 0: antes salia como si todo hubiera ido bien y
    rem pedia "volve a hacer doble clic para que Windows lo tome" - imposible,
    rem porque con PrependPath=0 Python nunca entra al PATH. El usuario
    rem reabria, se re-descargaba, se re-instalaba y volvia a salir por aca:
    rem un bucle que dejaba la carpeta elegida vacia (solo datos\ y temp\).
    pause & exit /b 1
  )
)

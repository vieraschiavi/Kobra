...
name: ecc
description: >
  Layer ECC (github.com/affaan-m/ECC v2.2.0) instalado en MV Kobra AI: reglas de estilo, testing,
...
1. Correr el gate real del repo y **pegar la salida cruda**, no un resumen:
   ```bash
   python3 verificar.py   # los 4 gates de CI
   ```
   Si `verificar.py` no está disponible, como mínimo: `python3 -m pytest -q tests/` — pero pytest solo es **un** gate de los cuatro.
